...
class Hyperparameters:
    ...
    # Ablation
    doc_mask_train = _e("DOC_MASK_TRAIN", 0, bool)
    doc_mask_eval = _e("DOC_MASK_EVAL", 0, bool)
    doc_boundary_id = _e("DOC_BOUNDARY_ID", -1, int)
    doc_mask_attn_backend = _e("DOC_MASK_ATTN_BACKEND", "auto")
    doc_mask_eval_log_coverage = _e("DOC_MASK_EVAL_LOG_COVERAGE", 1, bool)
    doc_mask_eval_strict_coverage = _e("DOC_MASK_EVAL_STRICT_COVERAGE", 1, bool)
    doc_mask_disable_seq_mix = _e("DOC_MASK_DISABLE_SEQ_MIX", 1, bool)
    doc_mask_compile = _e("DOC_MASK_COMPILE", 0, bool)
    doc_mask_compile_dynamic = _e("DOC_MASK_COMPILE_DYNAMIC", 1, bool)
...
class DistributedTokenLoader:
    def __init__(self, pattern: str, rank: int, world_size: int, device: torch.device,
                 doc_mask_train: bool = False, doc_boundary_id: int = -1):
        self.rank = rank
        self.world_size = world_size
        self.device = device
        self.doc_mask_train = doc_mask_train
        self.doc_boundary_id = int(doc_boundary_id)
        self.stream = TokenStream(pattern)

    def _sample_flat(self, per_rank_span: int, seq_len: int) -> tuple[Tensor, Tensor]:
    ...
    def next_batch(self, global_tokens: int, seq_len: int, grad_accum_steps: int) -> tuple[Tensor, Tensor]:
        ...
        x, y = self._sample_flat(per_rank_span, seq_len)
...
class Rotary(nn.Module):
    ...
    def forward(self, seq_len, device, dtype, pos_ids: Tensor | None = None):
        if pos_ids is not None:
            freqs = pos_ids.to(device=device, dtype=self.inv_freq.dtype)[..., None] * self.inv_freq.to(device)[None, None, :]
            return freqs.cos().unsqueeze(2).to(dtype=dtype), freqs.sin().unsqueeze(2).to(dtype=dtype)
        if self.no_cache:
...
class CausalSelfAttention(nn.Module):
    def __init__(self, dim, num_heads, num_kv_heads, rope_base, qk_gain_init,
        ...
        self.doc_mask_attn_backend = "auto"

    def _compute_segment_pos_ids(self, segment_ids: Tensor) -> Tensor:
        bsz, seqlen = segment_ids.shape
        idx = torch.arange(seqlen, device=segment_ids.device, dtype=torch.int32).unsqueeze(0).expand(bsz, seqlen)
        is_start = torch.ones_like(segment_ids, dtype=torch.bool)
        is_start[:, 1:] = segment_ids[:, 1:] != segment_ids[:, :-1]
        start_markers = torch.where(is_start, idx, torch.zeros_like(idx))
        segment_start = torch.cummax(start_markers, dim=1).values
        return (idx - segment_start).to(torch.int32)

    def _build_varlen_packing(self, q: Tensor, k: Tensor, v: Tensor, segment_ids: Tensor):
        bsz, seqlen = segment_ids.shape
        total_tokens = bsz * seqlen
        start_mask = torch.ones_like(segment_ids, dtype=torch.bool)
        start_mask[:, 1:] = segment_ids[:, 1:] != segment_ids[:, :-1]
        starts = torch.nonzero(start_mask.reshape(-1), as_tuple=False).squeeze(-1)
        if starts.numel() == 0:
            return None
        starts = starts.to(dtype=torch.int32, device=q.device)
        ends = torch.empty_like(starts)
        ends[:-1] = starts[1:]
        ends[-1] = total_tokens
        cu = torch.empty(starts.numel() + 1, dtype=torch.int32, device=q.device)
        cu[:-1] = starts
        cu[-1] = total_tokens
        q_flat = q.reshape(total_tokens, q.size(2), q.size(3)).contiguous()
        k_flat = k.reshape(total_tokens, k.size(2), k.size(3)).contiguous()
        v_flat = v.reshape(total_tokens, v.size(2), v.size(3)).contiguous()
        max_seqlen = seqlen
        return q_flat, k_flat, v_flat, cu, max_seqlen
    ...
    def _flash_attn_varlen(
        ...
        v: Tensor,
        segment_ids: Tensor,
        segment_cu_seqlens: Tensor | None = None,
        ...
        if segment_cu_seqlens is not None:
            ...
            k_flat = k.reshape(total_tokens, k.size(2), k.size(3)).contiguous()
            v_flat = v.reshape(total_tokens, v.size(2), v.size(3)).contiguous()
            cu = segment_cu_seqlens.to(dtype=torch.int32, device=q.device)
            use_max_seqlen = seqlen if max_seqlen is None else int(max_seqlen)
        else:
            packed = self._build_varlen_packing(q, k, v, segment_ids)
            if packed is None:
                return torch.zeros_like(q)
            q_flat, k_flat, v_flat, cu, use_max_seqlen = packed
        y_flat = flash_attn_varlen_func(
            q_flat, k_flat, v_flat,
            cu, cu, use_max_seqlen, use_max_seqlen,
            causal=True,
        )
        return y_flat.reshape_as(q)

    def _sdpa_doc_mask(self, q: Tensor, k: Tensor, v: Tensor, segment_ids: Tensor) -> Tensor:
        bsz, seqlen = segment_ids.shape
        ...
        causal = torch.tril(torch.ones(seqlen, seqlen, dtype=torch.bool, device=q.device)).unsqueeze(0)
        same_seg = segment_ids.unsqueeze(1) == segment_ids.unsqueeze(2)
        allowed = same_seg & causal
        attn_bias = torch.zeros((bsz, 1, seqlen, seqlen), dtype=q.dtype, device=q.device)
        attn_bias = attn_bias.masked_fill(~allowed.unsqueeze(1), float("-inf"))
        y_t = F.scaled_dot_product_attention(q_t, k_t, v_t, attn_mask=attn_bias, is_causal=False)
    ...
    def forward(
        ...
        segment_pos_ids: Tensor | None = None,
        segment_cu_seqlens: Tensor | None = None,
        ...
        seg_ids = segment_ids if (use_doc_mask and segment_ids is not None) else None
        if seg_ids is None:
            pos_ids = None
        elif segment_pos_ids is not None:
            pos_ids = segment_pos_ids
        else:
            pos_ids = self._compute_segment_pos_ids(seg_ids)
        cos, sin = self.rotary(seqlen, x.device, q.dtype, pos_ids=pos_ids)
        q, k = apply_rotary_emb(q, cos, sin), apply_rotary_emb(k, cos, sin)
        q = q * self.q_gain.to(dtype=q.dtype)[None, None, :, None]
        backend = self.doc_mask_attn_backend
        if backend == "auto":
            backend = "varlen" if flash_attn_varlen_func is not None else "sdpa"
        if self.diff_attn:
            ...
            elif backend == "varlen" and flash_attn_varlen_func is not None:
                y1 = self._flash_attn_varlen(
                    q1, k1, v1, seg_ids,
                    segment_cu_seqlens=segment_cu_seqlens, max_seqlen=seqlen,
                )
                y2 = self._flash_attn_varlen(
                    q2, k2, v2, seg_ids,
                    segment_cu_seqlens=segment_cu_seqlens, max_seqlen=seqlen,
                )
            else:
                y1 = self._sdpa_doc_mask(q1, k1, v1, seg_ids)
                y2 = self._sdpa_doc_mask(q2, k2, v2, seg_ids)
            ...
            if seg_ids is None:
                ...
                y = self._flash_attn_varlen(
                    q, k, v, seg_ids,
                    segment_cu_seqlens=segment_cu_seqlens, max_seqlen=seqlen,
                )
            else:
                y = self._sdpa_doc_mask(q, k, v, seg_ids)
...
class Block(nn.Module):
    ...
    def forward(
        ...
        segment_cu_seqlens: Tensor | None = None,
        use_doc_mask: bool = False,
        disable_seq_mix: bool = True,
    ) -> Tensor:
        mix = self.resid_mix.to(dtype=x.dtype)
        x = mix[0] * x + mix[1] * x0
        n = self.attn_norm(x)
        x = x + self.attn_scale.to(dtype=x.dtype) * self.attn(
            n,
            segment_ids=segment_ids,
            segment_pos_ids=segment_pos_ids,
        ...
        if self.smear is not None and not (use_doc_mask and disable_seq_mix):
            x = self.smear(x)
        return x
...
class GPT(nn.Module):
    def __init__(self, vocab_size, num_layers, model_dim, num_heads, num_kv_heads, mlp_mult,
                 ...
                 doc_mask_attn_backend: str = "auto"):
        super().__init__()
        ...
        self.doc_mask_attn_backend = doc_mask_attn_backend
        ...
        for block in self.blocks:
            block.attn.doc_mask_attn_backend = self.doc_mask_attn_backend
    ...
    def forward(
        ...
        x0 = x
        segment_ids = None
        segment_pos_ids = None
        segment_cu_seqlens = None
        if use_doc_mask and doc_boundary_id >= 0:
            boundary = (input_ids == int(doc_boundary_id)).to(torch.int32)
            segment_ids = torch.cumsum(boundary, dim=1)
            bsz, seqlen = segment_ids.shape
            idx = torch.arange(seqlen, device=segment_ids.device, dtype=torch.int32).unsqueeze(0).expand(bsz, seqlen)
            is_start = torch.ones_like(segment_ids, dtype=torch.bool)
            is_start[:, 1:] = segment_ids[:, 1:] != segment_ids[:, :-1]
            start_markers = torch.where(is_start, idx, torch.zeros_like(idx))
            segment_start = torch.cummax(start_markers, dim=1).values
            segment_pos_ids = (idx - segment_start).to(torch.int32)
            start_flat = torch.nonzero(is_start.reshape(-1), as_tuple=False).squeeze(-1).to(torch.int32)
            if start_flat.numel() > 0:
                segment_cu_seqlens = torch.empty(
                    start_flat.numel() + 1, dtype=torch.int32, device=segment_ids.device
                )
                segment_cu_seqlens[:-1] = start_flat
                segment_cu_seqlens[-1] = input_ids.numel()
        ...
        for i in range(self.num_encoder_layers):
            for _ in range(max(1, self.training_depth_recurrence)):
                x = self.blocks[i](
                    ...
                    segment_cu_seqlens=segment_cu_seqlens,
                    use_doc_mask=use_doc_mask,
                    disable_seq_mix=disable_seq_mix,
                )
            skips.append(x)
        ...
        if self.refiner is not None and not (use_doc_mask and disable_seq_mix):
        ...
        if self.training and len(self.mtp_heads) > 0 and not (use_doc_mask and disable_seq_mix):
...
def _expected_eval_tokens_non_sliding(val_tokens: Tensor, seq_len: int) -> int:
    return int(((val_tokens.numel() - 1) // seq_len) * seq_len)

def _expected_eval_tokens_sliding(total_tokens: int, seq_len: int, stride: int) -> int:
    starts = list(range(0, total_tokens - seq_len, stride))
    total = 0
    for s in starts:
        total += seq_len if s == 0 else stride
    return int(total)
...
def eval_val(args, model, rank, world_size, device, grad_accum_steps, val_tokens,
    ...
    use_doc_mask_eval = bool(args.doc_mask_eval) and int(args.doc_boundary_id) >= 0
    expected_total_tokens = _expected_eval_tokens_non_sliding(val_tokens, seq_len)
    all_starts = list(range(0, expected_total_tokens, seq_len))
    my_starts = all_starts[rank::world_size]
    local_expected_tokens = len(my_starts) * seq_len

    model.eval()
    with torch.inference_mode():
        for i in range(0, len(my_starts), local_batch_seqs):
            batch_starts = my_starts[i:i + local_batch_seqs]
            if not batch_starts:
                continue
            ...
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                batch_loss = model(
                    x, y, temperature=temperature,
                    use_doc_mask=use_doc_mask_eval, doc_boundary_id=int(args.doc_boundary_id),
                    disable_seq_mix=bool(args.doc_mask_disable_seq_mix),
                ).detach()
            n = float(y.numel())
    ...
    global_token_count = float(token_count.item())
    if rank == 0 and args.doc_mask_eval_log_coverage:
        mode = "doc_mask_eval" if use_doc_mask_eval else "standard_eval"
        print(
            f"coverage:{mode} expected_tokens:{expected_total_tokens} scored_tokens:{int(global_token_count)} "
            f"local_expected_tokens:{local_expected_tokens} local_scored_tokens:{int(local_token_count)}"
        )
    if float(token_count.item()) == 0.0:
        raise ValueError("eval_val: no tokens scored; check DOC_BOUNDARY_ID and DOC_MASK_EVAL settings.")
    if use_doc_mask_eval and args.doc_mask_eval_strict_coverage and global_token_count + 0.5 < expected_total_tokens:
        raise ValueError(
            f"eval_val coverage mismatch: scored {global_token_count:.0f} < expected {expected_total_tokens} "
            f"under doc-masked eval."
        )
...
def eval_val_sliding(args, model, rank, world_size, device, grad_accum_steps, val_tokens,
    ...
    my_starts = all_starts[rank::world_size]
    use_doc_mask_eval = bool(args.doc_mask_eval) and int(args.doc_boundary_id) >= 0
    expected_total_tokens = _expected_eval_tokens_sliding(total_tokens, seq_len, stride)
    local_expected_tokens = sum(seq_len if s == 0 else stride for s in my_starts)
        ...
        for i in range(0, len(my_starts), batch_size):
            ...
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                per_token_loss = model(
                    x, y, reduction="none", temperature=temperature,
                    use_doc_mask=use_doc_mask_eval, doc_boundary_id=int(args.doc_boundary_id),
                    disable_seq_mix=bool(args.doc_mask_disable_seq_mix),
                ).detach()
    ...
    global_token_count = float(token_count.item())
    if rank == 0 and args.doc_mask_eval_log_coverage:
        mode = "doc_mask_sliding_eval" if use_doc_mask_eval else "sliding_eval"
        print(
            f"coverage:{mode} expected_tokens:{expected_total_tokens} scored_tokens:{int(global_token_count)} "
            f"local_expected_tokens:{int(local_expected_tokens)} local_scored_tokens:{int(local_token_count)}"
        )
    if float(token_count.item()) == 0.0:
        raise ValueError("eval_val_sliding: no tokens scored; check DOC_BOUNDARY_ID and DOC_MASK_EVAL settings.")
    if use_doc_mask_eval and args.doc_mask_eval_strict_coverage and global_token_count + 0.5 < expected_total_tokens:
        raise ValueError(
            f"eval_val_sliding coverage mismatch: scored {global_token_count:.0f} < expected {expected_total_tokens} "
            f"under doc-masked eval."
        )
    val_loss = loss_sum / token_count
    bpb = (val_loss.item() / math.log(2.0)) * (token_count.item() / byte_count.item())
...
def main() -> None:
    ...
    resolved_doc_boundary_id = int(args.doc_boundary_id)
    if resolved_doc_boundary_id < 0:
        bos_id = int(sp.bos_id())
        if bos_id >= 0:
            resolved_doc_boundary_id = bos_id
    args.doc_boundary_id = resolved_doc_boundary_id
    if (args.doc_mask_train or args.doc_mask_eval) and args.doc_boundary_id < 0:
        log0("warning: doc masking enabled but no boundary token resolved; masking is effectively disabled.")
    log0(
        f"doc_mask_train:{int(args.doc_mask_train)} doc_mask_eval:{int(args.doc_mask_eval)} "
        f"doc_boundary_id:{args.doc_boundary_id} backend:{args.doc_mask_attn_backend} "
        f"disable_seq_mix:{int(args.doc_mask_disable_seq_mix)}",
        console=False,
    )
    if args.doc_mask_train or args.doc_mask_eval:
        resolved_backend = args.doc_mask_attn_backend
        if resolved_backend == "auto":
            resolved_backend = "varlen" if flash_attn_varlen_func is not None else "sdpa"
        log0(
            f"doc_mask_backend_resolved:{resolved_backend} "
            f"flash_attn_varlen_available:{int(flash_attn_varlen_func is not None)}",
            console=False,
        )
    if (args.doc_mask_train or args.doc_mask_eval) and args.doc_mask_disable_seq_mix:
        log0("doc_mask note: smear/refiner/MTP are disabled while doc masking is active.", console=False)
    val_tokens = ld_val(args.val_files, args.train_seq_len).to(torch.int64)
    ...
    base_model = GPT(
        ...
        doc_mask_attn_backend=args.doc_mask_attn_backend,
    ).to(device).bfloat16()
    ...
    doc_mask_active = bool(args.doc_mask_train or args.doc_mask_eval)
    use_compile = (not doc_mask_active) or bool(args.doc_mask_compile)
    if use_compile:
        compile_kwargs = {}
        if args.compile_mode != "default":
            compile_kwargs["mode"] = args.compile_mode
        if doc_mask_active and args.doc_mask_compile_dynamic:
            compile_kwargs["dynamic"] = True
        compiled_model = torch.compile(base_model, **compile_kwargs)
        if doc_mask_active and args.doc_mask_compile:
            log0(
                f"doc_mask active: forcing torch.compile (dynamic={int(args.doc_mask_compile_dynamic)}).",
                console=False,
            )
    else:
        compiled_model = base_model
        log0("doc_mask active: skipping torch.compile to keep masking path stable.", console=False)
    ...
    train_loader = DistributedTokenLoader(
        ...
        doc_mask_train=args.doc_mask_train, doc_boundary_id=args.doc_boundary_id,
    )
        ...
        for ws in range(args.warmup_steps):
            ...
            for mi in range(grad_accum_steps):
                ...
                with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                    loss = model(
                        x, y,
                        use_doc_mask=bool(args.doc_mask_train), doc_boundary_id=int(args.doc_boundary_id),
                        disable_seq_mix=bool(args.doc_mask_disable_seq_mix),
                    )
